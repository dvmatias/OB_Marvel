package com.cmdv.data.mappers

import com.cmdv.data.entities.SerieEntity
import com.cmdv.domain.base.BaseMapper
import com.cmdv.domain.models.SerieModel

private const val DEFAULT_INT = 1
private const val DEFAULT_STRING = ""

/**
 * Data mapper class. Utility class to map data between [SerieEntity] and [SerieModel] class types.
 */
object SerieMapper : BaseMapper<SerieEntity, SerieModel>() {

    override fun transformEntityToModel(e: SerieEntity): SerieModel =
        SerieModel(
            id = e.id ?: DEFAULT_INT,
            description = e.description ?: DEFAULT_STRING,
            transformThumbnail(e.thumbnail),
            title = e.title ?: DEFAULT_STRING
        )

    private fun transformThumbnail(thumbnail: SerieEntity.ThumbnailEntity?): String {
        val path = thumbnail?.path ?: DEFAULT_STRING
        val extension = thumbnail?.extension ?: DEFAULT_STRING
        return if (path.isNotEmpty() && extension.isNotEmpty()) {
            "$path.$extension"
        } else {
            DEFAULT_STRING
        }
    }
}